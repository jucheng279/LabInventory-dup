/*
# Add IF (Item Folder) prefix support to ai_resolve_codes

1. Modified Functions
   - `ai_resolve_codes`: Added a new WHEN 'IF' branch to resolve item folder 
     short codes (IF1, IF2, ...) into their UUIDs and parent hierarchy.
   
2. Details
   - For an IF code, looks up the item_folder by ai_code within the workspace 
     (joining through locations), and returns uuid, name, type='item_folder',
     location_id, sublocation_id, position_id.
   - Also adds folder_id to the item (I) code resolution so the client can 
     navigate to the containing sheet when clicking an item.

3. Important Notes
   - The IF prefix is two characters, same pattern as PR and EX.
   - Security: function remains SECURITY DEFINER, REVOKE/GRANT unchanged.
*/

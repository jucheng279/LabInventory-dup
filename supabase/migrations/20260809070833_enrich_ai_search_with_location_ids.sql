/*
# Enrich AI search results with full location path IDs

1. Modified Functions
   - `ai_search_inventory`: Changed `location_breadcrumb` field from a plain string
     to the full JSON object returned by `ai_get_location_breadcrumb`, which includes
     both the human-readable `breadcrumb` string AND a `path` array containing the
     UUID and name of each location level. This enables the AI to embed navigation
     links with real IDs in its responses.

   - `ai_list_expiring_inventory`: Same change — returns full location object instead
     of just the breadcrumb string.

   - `ai_list_low_stock_items`: Same change — returns full location object instead of
     just the breadcrumb string.

2. Security
   - No security changes. All functions retain their existing auth checks.

3. Important Notes
   - The `ai_get_location_breadcrumb` helper already returns both `path` (array of
     {id, name, type} objects) and `breadcrumb` (string). Previously only the
     `breadcrumb` text was extracted. Now the full object is passed through.
   - The AI system prompt has been updated to use these IDs in navigation link syntax.
*/

-- Update ai_search_inventory to return full location objects
...
-- Revoke public, grant only authenticated (preserve existing security)

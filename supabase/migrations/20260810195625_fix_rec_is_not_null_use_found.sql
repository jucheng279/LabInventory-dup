/*
# Fix rec IS NOT NULL bug in ai_resolve_codes — use FOUND instead

1. Bug Description
   - In PostgreSQL, `record IS NOT NULL` returns TRUE only when ALL fields
     in the record are non-null. Boxes at the location layer have
     sublocation_id=NULL and position_id=NULL, so `rec IS NOT NULL`
     evaluates to FALSE even though the query found a valid row.
   - This caused all B codes and cell refs to silently fail to resolve,
     while L codes (with all non-null fields) worked fine.

2. Fix
   - Replace every `IF rec IS NOT NULL THEN` with `IF FOUND THEN`.
     `FOUND` is the idiomatic PL/pgSQL way to check whether
     `SELECT INTO` returned a row, regardless of null field values.

3. Cleanup
   - Removed all RAISE LOG debug statements added in the previous

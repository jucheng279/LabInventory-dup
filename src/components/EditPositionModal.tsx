...
import { PositionWithStats, UpdatePositionData } from '../services/positionService';
...
interface EditPositionModalProps {
  ...
  onClose: () => void;
  onUpdate: (positionId: string, data: UpdatePositionData) => void;
  onDelete: () => void;
}
...
const EditPositionModal: React.FC<EditPositionModalProps> = ({
  ...
  const [accentColor, setAccentColor] = useState<string | null>(position.accent_color || '#6b7280');
  const [iconId, setIconId] = useState<string | null>(position.icon_id || null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onUpdate(position.id, {
        ...
        location_type: position.location_type, icon_id: iconId,
      });
    } finally {

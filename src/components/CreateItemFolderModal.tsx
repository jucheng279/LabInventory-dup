...
import { X, Table2, Plus, Trash2, ChevronLeft, ChevronRight, Calendar, CalendarClock, Type, GripVertical, Lock, List } from 'lucide-react';
...
interface CreateItemFolderModalProps {
  onClose: () => void;
  onCreate: (data: { name: string; description: string; accent_color: string | null; icon_id?: string | null }, headers: FolderHeaderInput[]) => void;
  lockedHeaders?: FolderHeaderInput[];
}
...
const CreateItemFolderModal: React.FC<CreateItemFolderModalProps> = ({ onClose, onCreate, lockedHeaders }) => {
  ...
  const lockedCount = lockedHeaders?.length || 0;
  const [headers, setHeaders] = useState<HeaderEntry[]>(() => {
    if (lockedHeaders && lockedHeaders.length > 0) {
      return lockedHeaders.map(h => ({ name: h.name, type: h.type, presetOptions: [] }));
    }
    ...
    <ModalFrame onClose={onClose} allowOverflow>
      <div className="flex-shrink-0 flex items-center justify-between p-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Create Item Sheet</h2>
          </div>
        </div>
        <button
          onClick={onClose}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  type="text"
                  id="folderName"
                  data-tutorial-id="create-folder-name-input"
                  value={name}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
              <div>
                ...
                <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                  {headers.map((header, index) => {
                    ...
                    const canRemove = !isLocked && headers.length > 1;
                    return (
                      ...
                      >
                        ...
                        <input
                          ...
                          readOnly={isLocked}
                          className={`flex-1 min-w-0 px-3 py-2 border rounded-lg transition-shadow text-sm ${
                            isLocked
                              ? 'border-blue-200 bg-blue-50/60 text-gray-700 cursor-default'
                              : 'border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent'
                          }`}
                        />
                        {isLocked ? (
                          <span className="p-1.5 flex-shrink-0 text-blue-400" title="Required header from box">
                            <Lock size={14} />
                          </span>
                        ) : (
                          <button
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  data-tutorial-id="create-folder-save-btn"
...
export default CreateItemFolderModal;

...
import { CreateSublocationData } from '../services/sublocationService';
...
interface CreateSublocationModalProps {
  locationId: string;
  onClose: () => void;
  onCreate: (data: CreateSublocationData) => void;
}

const CreateSublocationModal: React.FC<CreateSublocationModalProps> = ({
  locationId,
  onClose,
  onCreate,
}) => {
    ...
    <ModalFrame onClose={onClose}>
      <div className="flex-shrink-0 flex items-center justify-between p-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-gray-900">Add Sub-Location</h2>
        </div>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
          ...
          />
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                {isSubmitting ? 'Adding...' : 'Add Sub-Location'}
              </button>
            </div>
          </form>
    </ModalFrame>
